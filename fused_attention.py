...
from shard_map import shard_map, P, Mesh
...
if __name__ == "__main__":
  ...
  dtype = jnp.float16
  batch, seq_len, n_heads, head_dim = 256, 1024, 64, 64
  ...
  o_ref = mha(q, k, v)
  o_ref.block_until_ready()


  q_s, k_s, v_s = jax.device_put((q, k, v), jax.sharding.NamedSharding(
      mesh, P('x', None, 'y', None)))

  o = sharded_mha(q_s, k_s, v_s)
  o.block_until_ready()


  #o_ref = mha_reference(q, k, v)
  print("Checking tols...")
  is_close = jnp.allclose(o, o_ref, atol=0.01, rtol=0.01).block_until_ready()
  assert is_close
  ...
  duration = timeit.timeit(lambda: sharded_mha(q_s, k_s, v_s).block_until_ready(),
                           number=n_trials)
  print(f"Sharded Attention: {duration / n_trials * 1000:.2f}ms")
