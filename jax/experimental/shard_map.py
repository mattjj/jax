...
def _check_names(names: Sequence[AxisNames], avals: Sequence[core.ShapedArray]
  ...
  assert len(names) == len(avals), breakpoint()
...
class ShardMapTrace(core.Trace):
  ...
  def to_val_rep_pair(self, val):
    if isinstance(val, ShardMapTracer):
      ...
      return val_, frozenset(self.mesh.axis_names)
...
def _rep_to_vma(mesh, auto, rep: frozenset[AxisName]) -> frozenset[AxisName]:
  assert rep is not None, breakpoint()
...
def _shard_map_partial_eval(trace: pe.JaxprTrace, shard_map_p,
  ...
  def known_out_names():
    ...
    res_names = [{0: tuple(i for i in mesh.axis_names if i in a.vma)}
                 for a in res_avals]
    out_names = (*out_known_names, *res_names)
    return out_names
  ...
  res_avals_ = [in_consts[f1].aval if f1 is not None else
                out[f2].aval if f2 is not None else
                next(res_avals_iter) for f1, f2 in zip(in_fwd, out_fwd)]
  res_names = [known_in_names[f1] if f1 is not None else
               known_out_names_[f2] if f2 is not None else
               {0: tuple(i for i in mesh.axis_names if i in a.vma)}
               for f1, f2, a in zip(in_fwd, out_fwd, res_avals_)]
...
def _shard_map_linearize(trace, shard_map_p, f: lu.WrappedFun,
  ...
  tangent_params = dict(
      ...
      check_rep=True, rewrite=rewrite, auto=auto)
