...
def _maybe_check_special(prim, params, outs):
  if not config.debug_nans.value: return
  ...
  try:
    ...
    name = params.pop('name', prim.name) if prim is pjit.pjit_p else prim.name
    raise FloatingPointError(f"invalid value ({e.ty}) encountered in shard_map "
                             f"when applying {name}") from None
...
class ShardMapTrace(core.Trace):
  ...
  def process_primitive(self, prim, tracers, params):
    ...
    if eager_rule:
      ...
      _maybe_check_special(prim, params, out_vals)
