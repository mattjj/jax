...
class CustomVJPTraced(HiPrim):
  ...
  def remat(self, trace, *args):  # pyrefly: ignore[bad-param-name-override]
    ...
    n_out = len(self.out_avals_flat)
    ensure_unk = (False,) * n_out + (True,) * (len(jaxpr.outvars) - n_out)
    known, staged, _, _, num_res = pe.partial_eval_jaxpr_custom(
        jaxpr, (False,) * len(jaxpr.invars), True, ensure_unk, True, saveable)
    ...
    outs = core.eval_jaxpr(known, known.consts, *fc_flat, *args_flat)
    out_flat, saved = outs[:n_out], outs[n_out:]
    ...
    return tree_unflatten(self.out_tree, out_flat), rem
