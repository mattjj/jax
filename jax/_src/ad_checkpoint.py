...
def checkpoint_name_fwd(x, name):
  """Like :func:`checkpoint_name`, but for use inside a ``custom_vjp`` fwd rule.

  A ``custom_vjp`` fwd rule is replayed on the forward pass but never
  differentiated through (its derivatives flow via the bwd rule). A value
  named with ``checkpoint_name_fwd`` that a checkpoint ``policy`` selects to
  save is therefore stashed directly, without the recompute machinery that
  :func:`checkpoint_name` sets up for values on the primal (differentiated)
  path. Using plain ``checkpoint_name`` inside a fwd rule still produces
  correct values and gradients, but can save extra residuals; use this
  variant there. In primal code, use :func:`checkpoint_name`.
  """
  if config.remat3.value:
...
class CheckpointNameFwd(HiPrim):
  # For values named inside custom_vjp fwd rules, which are replayed primally
  # and never differentiated through. Saved/offloaded values are staged
  # directly rather than via a PrimalLeftTangentRight pair: the pair exists
  # only to route a tangent through a recompute branch, and a fwd rule has no
  # tangent to route, so the pair's recompute operand would only drag its
  # inputs in as spurious saved residuals.
  name: str
