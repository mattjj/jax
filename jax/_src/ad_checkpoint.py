...
class CustomRemat(HiPrim):
  ...
  # The reverse-mode rules below are reachable only when an application is
  # differentiated with no enclosing remat (e.g. bare jax.grad): under
  # jax.remat, the remat rule above dissolves the application before AD sees
  # it (out comes from f1, and the replay differentiates the helper).
